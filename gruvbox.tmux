...
main() {
  ...
  # NOTE: Checking for the value of @gruvbox_window_tabs
  local wt_enabled
  wt_enabled="$(get_tmux_option "@gruvbox_window_tabs" "off")"
  readonly wt_enabled
 ...
 #setw -g window-status-current-format "#[fg=colour232,bg=$thm_orange] #I #[fg=colour255,bg=colour237] #(echo '#{pane_current_path}' | rev | cut -d'/' -f-2 | rev) "
  ...
  # NOTE: With the @gruvbox_window_tabs set to on, we're going to
